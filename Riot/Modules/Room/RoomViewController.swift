...
extension RoomViewController: ShareContactDelegate {
    func shareUPContacts(sharedContacts: NSMutableAttributedString) {
        guard let inputToolbar = inputToolbar else {
            return
        }
        inputToolbar.attributedTextMessage = sharedContacts
    }
}
...
// Rum added replace occurrence
        if inputToolbar.attributedTextMessage.length > 0 {
            if #available(iOS 15.0, *) {
                ...
                newAttributedString.appendString(roomMember.displayname.count > 0 ? roomMember.displayname : roomMember.userId.replacingOccurrences(of: ServerURLs().upHomeServerUrl, with: ""))
            }
        ...
        self.setupRoomDataSource { roomDataSource in
            ...
            if self.inputToolbar?.sendMode == .reply, let eventModified = eventModified {
                ...
                roomDataSource.replaceAttributedTextMessage(
                    ...
                    failure: { _ in
//                        MXLog.error("[RoomViewController] sendAttributedTextMessage failed while updating event:", context: "\(eventModified.eventId)")
                })
            } else {
