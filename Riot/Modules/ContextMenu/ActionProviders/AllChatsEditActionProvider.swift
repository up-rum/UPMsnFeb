...
//        ])
    ...
    func updateMenuOptions(with session: MXSession?, completion: @escaping (UIMenu) -> Void) -> UIMenu {
//        self.parentSpace = parentSpace
//        self.rootSpaceCount = session?.spaceService.rootSpaces.count ?? 0
//        isInviteAvailable = false
//        isAddRoomAvailable = parentSpace == nil
...
//        }
    ...
    private var upShowSpaceAction: UIAction {
        UIAction(title: "My Spaces",
                 image: UIImage(systemName: "plus"),
                 attributes: isAddRoomAvailable ? [] : .disabled) { [weak self] action in
            guard let self = self else { return }

            self.delegate?.allChatsEditActionProvider(self, didSelect: .upShowSpace)
        }
    }
    private var upShowCallHistory: UIAction {
        UIAction(title: "Call History",
                 image: UIImage(systemName: "phone.fill"),
                 attributes: isAddRoomAvailable ? [] : .disabled) { [weak self] action in
            guard let self = self else { return }

            self.delegate?.allChatsEditActionProvider(self, didSelect: .upCallHistory)
        }
    }
