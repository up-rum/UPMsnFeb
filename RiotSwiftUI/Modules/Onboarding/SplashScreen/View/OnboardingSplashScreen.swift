...
//        Color("BgColor")
        GeometryReader { geometry in
            ZStack {
                ...
                VStack(spacing: 0) {
//                    Spacer()
                    Image(uiImage: #imageLiteral(resourceName: "SplashLogo"))
                        .resizable()
                    ...
                    Button {
                        ...
                        Text("Sign In")
                            .frame(width: geometry.size.width-40, height: 48, alignment: .center)
                            .foregroundColor(Color.black)
                            .background(LinearGradient(colors: [Color("Gradient1"),Color("Gradient2")], startPoint: .top, endPoint: .bottom))
                            .clipShape(RoundedRectangle(cornerRadius: 10))
                    }.padding(.top,60)
//                    .buttonStyle(PrimaryActionButtonStyle(: Color("SColor"), customtextColor: Color.black))
                    
                Button { viewModel.send(viewAction: .register) }
                label: {
                        Text("Register")
                }
                .frame(width: geometry.size.width-40, height: 48, alignment: .center)
//                .border(Color("SColor"), width: 1)
                .foregroundColor(Color.white)
//                .clipShape(RoundedRectangle(cornerRadius: 10))
                .overlay(
                        RoundedRectangle(cornerRadius: 10).stroke(Color("SColor"), lineWidth: 1)
                    )
                .padding(.top,20)
...
//                    }
                    ...
                    Text("Your privacy matters so let's keep things simple")
                        .font(.system(size: 14, weight: .medium ))
                    Text("We won't sell, trade or share your information")
                        .font(.system(size: 14, weight: .bold )).padding(.bottom,5)
                    Text((appVersion ?? "1.0.0") + " (release)")
                        .font(.system(size: 14, weight: .medium ))
                }
                .padding()
...
//         NavigationView{
//        VStack(spacing: 20) {
//            NavigationLink(destination: UPRegContentView(), tag: "A", selection: $selection) {  }
//
//            Button {
//
//                selection = "A"
...
struct PrimaryButton: View {
    ...
    var body: some View {
        Text(title)
            ...
            .padding()
            .background(
                LinearGradient(gradient: Gradient(colors: [Color("PrimaryColor"), Color("SColor")]), startPoint: .bottomLeading, endPoint: .topTrailing))
